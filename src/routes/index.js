...
router.get('/', (req, res) => {
  res.json({
    ...
    availableRoutes: [
      ...
      'POST /api/order-after-sales - 创建售后（用户权限）',
      'GET /api/order-after-sales - 获取当前用户的售后列表（用户权限）',
      'GET /api/order-after-sales/:id - 获取售后详情（用户权限）',
      'PUT /api/order-after-sales/:id - 更新售后（用户权限）',
      'DELETE /api/order-after-sales/:id - 删除售后（用户权限）',
      'GET /api/order-after-sales/admin/list - 获取所有售后列表（管理员权限）',
      'GET /api/order-after-sales/admin/:id - 获取售后详情（管理员权限）',
      'PUT /api/order-after-sales/admin/:id - 更新售后（管理员权限）',
      'PUT /api/order-after-sales/admin/:id/status - 修改售后状态（管理员权限）',
      'DELETE /api/order-after-sales/admin/:id - 删除售后（管理员权限）',
      'GET /api/statistics - 获取数据统计（管理员权限）',
      'GET /api/pdf-config - 获取PDF配置（管理员权限）',
      'POST /api/pdf-config - 创建/设置PDF配置（管理员权限）',
      'GET /api/pdf-config/all - 获取所有PDF配置列表（管理员权限）',
      'GET /api/pdf-config/:id - 获取PDF配置详情（管理员权限）',
      'PUT /api/pdf-config/:id - 更新PDF配置（管理员权限）',
