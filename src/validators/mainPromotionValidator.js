 ...
 * @FilePath: \showcase-backend-node\src\validators\mainPromotionValidator.js
 * @Description: 主推款式数据验证器
 */

class MainPromotionValidator {
  // 验证添加主推款式的请求数据
  static validateCreatePromotion(data) {
    const errors = [];
  ...
  static validateBatchUpdateSortOrder(data) {
    ...
    data.updates.forEach((update, index) => {
      if (!update.promotion_id) {
        errors.push(`第${index + 1}项：主推款式ID不能为空`);
      } else if (isNaN(parseInt(update.promotion_id)) || parseInt(update.promotion_id) <= 0) {
        errors.push(`第${index + 1}项：主推款式ID必须是正整数`);
      }
  ...
  static validateCreate(req, res, next) {
    const validation = MainPromotionValidator.validateCreatePromotion(req.body);
  ...
  static validateUpdateSort(req, res, next) {
    const validation = MainPromotionValidator.validateUpdateSortOrder(req.body);
  ...
  static validateBatchUpdateSort(req, res, next) {
    const validation = MainPromotionValidator.validateBatchUpdateSortOrder(req.body);
  ...
  static validateIdParam(req, res, next) {
    const validation = MainPromotionValidator.validateId(req.params.id);
...
module.exports = MainPromotionValidator;
